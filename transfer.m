...
tex = double(imread('images/yogurt.jpg'));
% tex = imresize(tex, 0.5);
[tex_h, tex_w, ~] = size(tex);

tar = double(imread('images/lincoln.jpg'));
...
for i = 1:(floor(out_h/bsize))     % loops over rows
  for j = 1:(floor(out_w/bsize))   % loops over cols
